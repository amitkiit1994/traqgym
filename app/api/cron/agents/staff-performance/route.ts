...
import { run } from "@/lib/agents/staff-performance";
...
export async function GET(req: NextRequest) {
  ...
  const enabled = await getSetting("cron_staff_performance_enabled", "true");
  if (enabled !== "true") {
  ...
  try {
    ...
    console.error("[staff-performance cron] Error:", err);
