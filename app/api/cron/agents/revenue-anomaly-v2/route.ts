import { run } from "@/lib/agents/revenue-anomaly-v2";
...
export async function GET(request: Request) {
  ...
  const enabled = await getSetting("cron_revenue_anomaly_v2_enabled", "true");
  if (enabled !== "true") {
  ...
  try {
    ...
    console.error("[revenue-anomaly-v2 cron] Error:", err);
